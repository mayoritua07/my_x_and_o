...
class WifiPlay extends ConsumerStatefulWidget {
  const WifiPlay({super.key, required this.value, required this.cards});

  final String value;
  final List cards;

  @override
  ConsumerState<WifiPlay> createState() {
    return _GameScreenState();
  }
}

class _GameScreenState extends ConsumerState<WifiPlay> {
  late Player xPlayer;
  late Player oPlayer;
  late String value;
  late String myValue;
  ...
  void onBackCardTap() {
    if (xPositions.length + oPositions.length > 3) {
      ScaffoldMessenger.of(context).clearSnackBars();
      ScaffoldMessenger.of(context).showSnackBar(
    ...
    if (userActiveCard != null) {
      ScaffoldMessenger.of(context).clearSnackBars();
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("A card is already active")),
      );
      return;
    }
    setState(() {
